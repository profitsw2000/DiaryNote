...
class CategoryNotesDataSource(
    ...
    override fun loadInitial(params: LoadInitialParams, callback: LoadInitialCallback<NoteModel>) {
        ...
        compositeDisposable.add(
            database.noteDao.getUserNotesByCategory(userId, categoryId, params.requestedLoadSize, params.requestedStartPosition)
    ...
    override fun loadRange(params: LoadRangeParams, callback: LoadRangeCallback<NoteModel>) {
        notesState.postValue(NotesState.Loading)
        compositeDisposable.add(
            database.noteDao.getUserNotesByCategory(userId, categoryId, params.loadSize, params.startPosition)
                .subscribeOn(Schedulers.io())
                .subscribe(
                    {
